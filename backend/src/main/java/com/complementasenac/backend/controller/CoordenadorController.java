...
public class CoordenadorController {
    ...
    public ResponseEntity<?> decidirAtividade(@PathVariable String id, @RequestBody Map<String, Object> payload) {
        String status = payload.getOrDefault("status", "PENDENTE").toString().toUpperCase();
        Integer horasAprovadas = payload.get("horasAprovadas") instanceof Number n ? n.intValue() : null;
        String justificativa = payload.get("justificativa") == null ? null : payload.get("justificativa").toString();
        return coordenadorService.decidir(id, status, horasAprovadas, justificativa)
